 ...
 * backend, unauthenticated on the LAN). Best-effort and optimistic: the backend
 * `/progress` poll remains authoritative. Singleton so the link survives recomposition.
 */
interface EdgeProgressDataSource {
    /** Cold stream of live rep events for [serialNumber]; collecting opens the SSE connection. */
    fun observeProgress(serialNumber: String): Flow<LiveRepEvent>
}

@Singleton
class EdgeProgressDataSourceImpl @Inject constructor(
    @Named("edgeBaseUrl") private val edgeBaseUrl: String,
    @Named("edgeOkHttp") private val client: OkHttpClient,
    private val json: Json
) : EdgeProgressDataSource {

    override fun observeProgress(serialNumber: String): Flow<LiveRepEvent> = callbackFlow {
        val url = edgeBaseUrl.trimEnd('/') +
